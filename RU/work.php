...
<header class="header">
    <div class="container">
        <div class="header__menu_work">
            <a href="https://iamnikolay.ru/RU" class="header__link">На главную</a>
        </div>
...
<section class="work">
    <div class="container">
        ...
        $id = $_GET['id'];
        $result = $connect->query("SELECT * FROM `worksRU` WHERE `id` = '$id'");
        ...
        if($item['warning'] == 1){
            echo('
                        <div class="work__warning">
                            ...
                            <p>Возможно в данный момент я уже не занимаюсь поддержкой данного проекта, однако с кодом вы можете ознакомиться на <a href="' . $item['githubLink'] . '">гитхабе.</a></p>
                        </div>
                    ');
        }else{
            echo('
                        <div class="work__warning warning__figma-wrapper" style="padding: 39px; margin-top: 70px; cursor: pointer">
                            ...
                            <div class="work__warning-title" style="margin-bottom: 0px!important;">
                                <h2 style="margin: 0 auto!important; font-size: 48px; position: relative; left: 20px" class="work__figma">Ссылка на макет</h2>
                            </div>
                        </div>
                    ');
        }
